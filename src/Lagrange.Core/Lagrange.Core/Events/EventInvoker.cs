...
public sealed class EventInvoker(BotContext context) : IDisposable
{
    ...
    internal void PostEvent<T>(T ev) where T : EventBase => Task.Run(async () =>
    {
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            {
                // stdout is the runner JSON-RPC transport; diagnostics must
                // never corrupt its one-JSON-object-per-line framing.
                Console.Error.WriteLine($"Failed to post event: {ev}");
                return;
            }
